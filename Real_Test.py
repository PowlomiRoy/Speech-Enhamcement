...
class test_model:
    ...
    def loadmodel(self):
        if self.modelname == 'DCCTN':
            from Real_Network import DCCTN
    ...
    def main(self):
        ...
        for audiofile in tqdm(eval_files, desc='Decoidng Eval Files '):
            ...
            enh_audio = model(torch.FloatTensor(noisy).unsqueeze(0).to('cpu')).detach().cpu().numpy()
...
if __name__ == '__main__':
    ...
    modelname = 'DCCTN'
    modelfile = 'DCCTN-DADX-IEEE-SISDR+FreqLoss-epoch=01-val_loss=45.76.ckpt'
    Loss_function = 'SISDR+FreqLoss=45.76'
