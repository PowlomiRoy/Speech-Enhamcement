...
for Noise_type in Noise_type_all:
    for SNR in SNR_all:
        for i in range(Audio_sample):
            signal_path = rootdir + '/Dev/Clean/Clean_' + str(i + 11) + '.wav'
            noisy_path = rootdir + '/Dev/Noisy/Noisy_' + Noise_type + '_' + str(SNR) + '_dB_' + str(i + 11) + '.wav'
            L = noisy_path + " " + signal_path
            file.writelines(L)
            file.write('\n')
file.close()
